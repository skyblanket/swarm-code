...
# Runtime artifacts the binary links against. Without these as real
# prerequisites, a rebuilt swarmrt (new builtins, C fixes) silently did
# NOT trigger a relink — `make` said up-to-date while bin/swarm-code ran
# stale runtime code (bit us 2026-07-09 verifying a line-editor fix).
RUNTIME := $(wildcard $(SWARMRT)/bin/libswarmrt.a $(SWARMRT)/bin/swc)
...
$(BIN): $(SRC_DIR)/Version.sw $(SOURCES) $(RUNTIME)
	@mkdir -p bin
...
$(TESTBIN): $(SRC_DIR)/Version.sw $(SOURCES) $(RUNTIME)
	@mkdir -p bin
